...
class PatchEmbed(nn.Module):
    ...
    proj : nn.Conv2d
        Convolutional layer that does both the spitting into patches and their embedding.
...
class Attention(nn.Module):
    ...
    qkv : nn.Linear
        Linear projection for the q, k, v
    ...
    def forward(self,x):
        ...
        ----------
        x : torch.Tensor
            Shape '(n_samples, n_patches + 1(CLS), dim)'.
        Returns
        --------
        ...
        qkv = qkv.permute(2,0,3,1,4) # Change the order
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) # Initialized to zero
        self.pos_embed = nn.Parameter(
                torch.zeros(1, 1 + self.patch_embed.n_patches, embed_dim)
        ) # Learnable parameter, also for cls token
